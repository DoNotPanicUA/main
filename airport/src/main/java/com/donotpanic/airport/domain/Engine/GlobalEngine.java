...
//    private GlobalEngine globalEngine;
    ...
    GlobalEngine globalEngine(){
        return new GlobalEngine();
    }

    GlobalEngine(){
//        globalEngine = new GlobalEngine();
    ...
    private boolean checkAirportCoordinates(Coordinates coordinates){
        ...
        for (Airport airport : airports){
            if (GeoLocationService.calculateDistance(coordinates, getObjectCoordinates(airport)) <= engineSettings.getAirportMinimalDistance()){
                result = false;
                break;
            }
        }
    ...
    private boolean checkBuildingCoordinates(Coordinates coordinates){
        ...
        for (Coordinates existingCoordinates : buildings.keySet()){
            if (GeoLocationService.calculateDistance(coordinates, existingCoordinates) < engineSettings.getBuildingMinimalDistance()){
                System.out.println(GeoLocationService.calculateDistance(coordinates, existingCoordinates) + " is less than " +engineSettings.getBuildingMinimalDistance());
                result = false;
                break;
            }
        }
    ...
    public synchronized void registerAirport(Airport airport) throws InvalidCoordinatesException{
        ...
        for (int i = 0; i < 10000 ; i++){
            if (coordinates == null){
                 coordinates = coordinateFactory.getRandomCoordinates();
            }
    ...
    public synchronized void registerAirportBuilding(Airport airport, GlobalObject building, Direction direction, double distance) throws InvalidCoordinatesException{
        ...
        if (airportCoor != null){
            Coordinates objectCoordinates = GeoLocationService.calculateNewCoordinates(airportCoor, direction, distance);
    ...
    private void registerCoordinates(Coordinates coordinates, GlobalObject object) throws InvalidCoordinatesException{
        if (globalMap.containsKey(object)){
            ...
            drawEngine.addGlobalObject(object);
        }
    }
    ...
    public Coordinates moveObjectByDirection(MovableObject object, Actions moveAction, Direction direction) throws Throwable{
        ...
        Coordinates newCoordinates = coordinateFactory.getCoordinates(currentCoordinates.getX() + direction.getCoefX() * object.getSpeed() * moveAction.getDurationSec(),
    ...
    public boolean moveObjectToCoordinates(MovableObject object, Actions moveAction, Coordinates destination) throws Throwable{
        if (GeoLocationService.calculateDistance(destination,getObjectCoordinates(object)) <= object.getSpeed()*moveAction.getDurationSec()){
            moveAction.doAction();
            registerCoordinates(destination, object);
            /*Object reached destination coordinates*/
            return true;
        }else {
            moveObjectByDirection(object, moveAction, GeoLocationService.getDirection(destination, getObjectCoordinates(object)));
            return false;
        }
    }
